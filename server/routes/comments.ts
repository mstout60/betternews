...
export const commentsRouter = new Hono<Context>()
    .post("/:id", loggedIn, zValidator("param", z.object({ id: z.coerce.number() })), zValidator("form", createCommentSchema), async (c) => {
        ...
        const [comment] = await db.transaction(async (tx) => {
            ...
            const [updateParentComment] = await tx
                ...
                .returning({ commentsCount: commentsTable.commentCount });

            const [updatedPost] = await tx
                .update(postsTable)
                .set({ commentCount: sql`${postsTable.commentCount} + 1` })
                .where(eq(postsTable.id, postId))
                .returning({ commentsCount: postsTable.commentCount });
    ...
    .post("/:id/upvote", loggedIn, zValidator("param", z.object({ id: z.coerce.number() })), async (c) => {
        ...
        const points = await db.transaction(async (tx) => {
            ...
            const [updated] = await tx.update(commentsTable)
                .set({
            ...
            if (existingUpvote) {
                await tx
                    ...
                    .where(
                        eq(commentUpvotesTable.id, existingUpvote.id),
                    );
            } else {
                await tx
