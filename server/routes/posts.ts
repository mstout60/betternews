...
export const postRouter = new Hono<Context>()
    ...
    .get("/:id/comments", zValidator("param", z.object({ id: z.coerce.number() })), zValidator("query", paginationSchema.extend({ includeChildren: z.boolean({ coerce: true }).optional() })), async (c) => {
        const user = c.get("user");
        const { id } = c.req.valid("param");
